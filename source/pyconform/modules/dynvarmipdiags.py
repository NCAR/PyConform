...
def _init_wtem(time, levi, lat, wzm, vthzm, thzm):
    ...
    _wzm = -1.0 * numpy.einsum('ijk,j->ijk', wzm, levi100) / H

    wtem = numpy.zeros((ntime, nlevi, nlat), dtype='d')
...
def wtem(time, levi, lat, wzm, vthzm, thzm):
    ...
    wtem = -1.0 * numpy.einsum('ijk,j->ijk', wtem, H / levi100)
    return wtem
...
def vtem(time, levi, lat, vzm, vthzm, thzm):
    ...
    vtem = numpy.zeros((ntime, nlevi, nlat), dtype='d')
    for itime in range(ntime):
        dthdp = deriv(levi100, thzm[itime, :, :])
        psieddy = vthzm[itime, :, :] / dthdp
...
def utendvtem(time, levi, lat, uzm, vzm, vthzm, thzm):
    ...
    utendvtem = numpy.zeros((ntime, nlevi, nlat), dtype='d')
    for itime in range(ntime):
...
def utendwtem(time, levi, lat, uzm, wzm, vthzm, thzm):
    ...
    utendwtem = numpy.zeros((ntime, nlevi, nlat), dtype='d')
    for itime in range(ntime):
...
def _init_epfy(time, levi, lat, uzm, uvzm, vthzm, thzm):
    ...
    epfy = numpy.zeros((ntime, nlevi, nlat), dtype='d')
    for itime in range(ntime):
        dthdp = deriv(levi100, thzm[itime, :, :])
        dudp = deriv(levi100, uzm[itime, :, :])
        psieddy = vthzm[itime, :, :] / dthdp
...
def epfy(time, levi, lat, uzm, uvzm, vthzm, thzm):
    ...
    epfy = numpy.einsum('ijk,j->ijk', epfy, levi100) / p0
    return epfy
...
def _init_epfz(time, levi, lat, uzm, uwzm, vthzm, thzm):
    ...
    _uwzm = -1.0 * numpy.einsum('ijk,j->ijk', uwzm, levi100) / H

    epfz = numpy.zeros((ntime, nlevi, nlat), dtype='d')
    for itime in range(ntime):
        ucos = numpy.einsum('ij,j->ij', uzm[itime, :, :], coslat)
        dudphi = deriv(latrad, ucos, axis=1) / a
        dthdp = deriv(levi100, thzm[itime, :, :])
...
def utendepfd(time, levi, lat, uzm, uvzm, uwzm, vthzm, thzm):
    ...
    utendepfd = numpy.zeros((ntime, nlevi, nlat), dtype='d')
    for itime in range(ntime):
        depfydphi = deriv(latrad, epfy[itime, :, :]
                          * coslat, axis=1) / (a * coslat)
...
def psitem(time, levi, lat, vzm, vthzm, thzm):
    ...
    psitem = numpy.zeros((ntime, nlevi, nlat), dtype='d')
    vzmwithzero = numpy.zeros((nlevi + 1, nlat), dtype='d')
    ...
    for itime in range(ntime):
        ...
        for ilev in range(1, nlevi + 1):
            ...
            psitem[itime, ilev - 1,
                   :] = ((2 * numpy.pi * a * coslat) / g0) * (result - psieddy[ilev - 1, :])

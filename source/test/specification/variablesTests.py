...
class VariableTests(unittest.TestCase):
    ...
    def test_setting_definition_in_constructor_to_expression_saves_expression(self):
        v = self.spec.new_variable('v', definition='f(x)')
        self.assertEqual(v.definition, 'f(x)')

    def test_setting_definition_to_array_like_saves_array_like(self):
        vdef = [[1, 2], [3, 4], [5, 6]]
        v = self.spec.new_variable('v', definition=vdef)
    ...
    def test_setting_datatype_in_constructor_to_valid_value_saves_value(self):
        v = self.spec.new_variable('v', datatype='int')
        self.assertEqual(v.datatype, 'int')
    ...
    def test_setting_dimensions_in_constructor_to_tuple_saves_tuple(self):
        x = self.spec.new_dimension('x')
        y = self.spec.new_dimension('y')
        v = self.spec.new_variable('v', dimensions=('x', 'y'))
        self.assertEqual(v.dimensions, {'x': x, 'y': y})
    ...
    def test_setting_attributes_in_constructor_with_dict(self):
        v = self.spec.new_variable('v', attributes={'a': 'b'})
        self.assertEqual(v.attributes, {'a': 'b'})
    ...
    def test_setting_bounds_to_string(self):
        v_bnspec = self.spec.new_variable('v_bnspec')
        v = self.spec.new_variable('v')
        v.bounds = 'v_bnspec'
        self.assertEqual(v.bounds, v_bnspec)

    def test_coordinates(self):
        self.spec.new_dimension('i')
        self.spec.new_dimension('j')
        i = self.spec.new_variable('i')
        i.dimensions = ('i',)
        j = self.spec.new_variable('j')
        j.dimensions = ('j',)
        x = self.spec.new_variable('x')
        x.dimensions = ('i', 'j')
        y = self.spec.new_variable('y')
    ...
    def test_equal(self):
        spec1 = Specification()
        spec2 = Specification()
        spec1.new_dimension('x')
        spec2.new_dimension('x')
        v1 = spec1.new_variable('v')
        v1.datatype = 'float'
        v1.dimensions = ('x',)
        v2 = spec2.new_variable('v')
        v2.datatype = 'float'
        v2.dimensions = ('x',)

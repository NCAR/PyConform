...
class DimensionTests(unittest.TestCase):
    ...
    def test_setting_size_in_constructor_stores_value(self):
        d = self.spec.new_dimension('x', size=5)
        self.assertEqual(d.size, 5)
    ...
    def test_setting_is_unlimited_in_constructor_to_bool_saves_bool(self):
        d = self.spec.new_dimension('x', is_unlimited=True)
        self.assertTrue(d.is_unlimited)

    def test_setting_is_unlimited_to_non_bool_saves_bool_value(self):
        d = self.spec.new_dimension('x', is_unlimited=5)
        self.assertTrue(d.is_unlimited)

    def test_equal(self):
        spec1 = Specification()
        spec2 = Specification()
        d1 = spec1.new_dimension('x', size=4)
        d2 = spec2.new_dimension('x', size=4)
        self.assertEqual(d1, d2)

    def test_not_equal(self):
        spec1 = Specification()
        spec2 = Specification()
        d1 = spec1.new_dimension('x', size=3)
        d2 = spec2.new_dimension('y', size=4)

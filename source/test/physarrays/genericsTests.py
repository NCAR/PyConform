"""
Unit Tests for generic DataArray functions
...
import pyconform.physarrays.generics as fn
...
class FunctionsTests(unittest.TestCase):
    ...
    def assertDataArraysEqual(self, obj1, obj2):
        self.assertEqual(type(obj1), type(obj2))
        xr.testing.assert_equal(obj1, obj2)
        self.assertEqual(fn.get_name(obj1), fn.get_name(obj2))
        self.assertEqual(fn.get_cfunits(obj1), fn.get_cfunits(obj2))
        self.assertEqual(fn.get_positive(obj1), fn.get_positive(obj2))
    ...
    def test_is_equal_dataarray_with_matching_units_and_positive_is_true(self):
        ...
        self.assertEqual(fn.get_name(v1), fn.get_name(v2))
        self.assertEqual(fn.get_cfunits(v1), fn.get_cfunits(v2))
        self.assertEqual(fn.get_positive(v1), fn.get_positive(v2))
    ...
    def test_is_equal_dataarray_with_mismatched_units_is_false(self):
        ...
        xr.testing.assert_equal(v1, v2)
        self.assertEqual(fn.get_name(v1), fn.get_name(v2))
        self.assertNotEqual(fn.get_cfunits(v1), fn.get_cfunits(v2))
        self.assertEqual(fn.get_positive(v1), fn.get_positive(v2))
    ...
    def test_is_equal_dataarray_with_mismatched_name_is_false(self):
        ...
        self.assertEqual(type(v1), type(v2))
        xr.testing.assert_equal(v1, v2)
        self.assertNotEqual(fn.get_name(v1), fn.get_name(v2))
        self.assertEqual(fn.get_cfunits(v1), fn.get_cfunits(v2))
        self.assertEqual(fn.get_positive(v1), fn.get_positive(v2))
    ...
    def test_is_equal_dataarray_with_mismatched_positive_is_false(self):
        ...
        self.assertEqual(fn.get_name(v1), fn.get_name(v2))
        self.assertEqual(fn.get_cfunits(v1), fn.get_cfunits(v2))
        self.assertNotEqual(fn.get_positive(v1), fn.get_positive(v2))
    ...
    def test_convert(self):
        v = xr.DataArray([[1, 2], [3, 4]], name='v', dims=['x', 'y'],
                         coords=[[0, 1], [1, 2]], attrs={'units': 'g'})
        v_kg = fn.convert(v, 'kg')
        self.assertIsInstance(v_kg, xr.DataArray)
    ...
    def test_convert_time_referenced_with_diff_calendar_units_raises_units_error(self):
        v = xr.DataArray([[1, 2], [3, 4]], name='v', dims=['x', 'y'],
                         coords=[[0, 1], [1, 2]], attrs={'units': 'days since 1974-02-06',
                                                         'calendar': 'noleap'})
    ...
    def test_convert_time_referenced_with_same_calendar_units(self):
        v = xr.DataArray(0.0, name='v', attrs={'units': 'days since 1999-01-01',
                                               'calendar': 'noleap'})
        to_units = cu.Unit('hours since 2000-01-01', calendar='noleap')
        actual = fn.convert(v, to_units)
        expctd = xr.DataArray(-8760.0, name="convert(v, to='hours since 2000-01-01')",
                              attrs={'units': 'hours since 2000-01-01', 'calendar': 'noleap'})
        self.assertDataArraysEqual(actual, expctd)

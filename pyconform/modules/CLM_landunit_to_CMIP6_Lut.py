...
class CLM_landunit_to_CMIP6_Lut_Function(Function):
    ...
    def __init__(self, EFLX_LH_TOT, vegType, ntim, nlat, nlon, grid1d_ixy, grid1d_jxy, grid1d_lon,
        ...
        super(CLM_landunit_to_CMIP6_Lut_Function, self).__init__(
            EFLX_LH_TOT, vegType, ntim,
            nlat, nlon, grid1d_ixy, grid1d_jxy, grid1d_lon, grid1d_lat, land1d_lon, land1d_lat,
            land1d_ityplunit, land1d_active, land1d_wtgcell, landUse)
    ...
    def __getitem__(self, index):
        ...
        if index is None:
            if 'all' in vegType:
                return PhysArray(np.zeros((0, 4, 0, 0)),
                                 dimensions=[pntim.dimensions[0],
                                             landUse.dimensions[0],
                                             pnlat.dimensions[0],
                                             pnlon.dimensions[0]])
            else:
        ...
        new_name = 'CLM_landunit_to_CMIP6_Lut({}{}{}{}{}{}{}{}{}{}{}{}{})'.format(
            pEFLX_LH_TOT.name, pntim.name, pnlat.name, pnlon.name, pgrid1d_ixy.name,
            pgrid1d_jxy.name, pgrid1d_lon.name, pgrid1d_lat.name, pland1d_lon.name,
            pland1d_lat.name, pland1d_ityplunit.name, pland1d_active.name,
            pland1d_wtgcell.name)
